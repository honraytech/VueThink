  ...
  export default {
    data() {
      return {
        ...
        requireVerify: false,
        verifyUrl: '',
        verifyImg: window.HOST + 'index/base/getVerify',
        rules2: {
          username: [
    ...
    methods: {
      ...
      handleSubmit2(form) {
        ...
        this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            this.apiPost('index/base/login', data).then((res) => {
              if (res.code != 200) {
                this.loading = !this.loading
      ...
      checkIsRememberPwd() {
        if (Cookies.get('rememberPwd') === 'true') {
          ...
          this.apiPost('index/base/relogin', data).then((res) => {
    ...
    created() {
      ...
      this.apiPost('index/base/getConfigs').then((res) => {
        this.handelResponse(res, (data) => {
